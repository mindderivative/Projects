---
title: "Process material properties"
source: "https://docs.godotengine.org/en/stable/tutorials/3d/particles/process_material_properties.html#particle-flags"
created: 2026-06-20
description: "ParticleProcessMaterial properties Min, max, and curve properties, The properties in this material control how particles behave and change over their lifetime. A lot of them have Min, Max, and Curv..."
words: 3158
---
![ParticleProcessMaterial properties](https://docs.godotengine.org/en/stable/_images/particle_minmaxcurve1.webp)
...
> [!note] Note
> This page covers how to use ParticleProcessMaterial for 3D scenes specifically. For information on how to use it in a 2D Scene see [ParticleProcessMaterial 2D Usage](https://docs.godotengine.org/en/stable/tutorials/2d/particle_process_material_2d.html#doc-particle-process-material-2d).
...
The `Align Y` property aligns each particle's Y-axis with its velocity. Enabling this property is the same as setting the [Transform Align](https://docs.godotengine.org/en/stable/tutorials/3d/particles/properties.html#doc-3d-particles-properties-draw) property to `Y to Velocity`.

The `Rotate Y` property works with the properties in the and groups to control particle rotation. `Rotate Y` has to be enabled if you want to apply any rotation to particles. The exception to this is any particle that uses the [Standard Material](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d) where the `Billboard` property is set to `Particle Billboard`. In that case, particles rotate even without `Rotate Y` enabled.

When the `Disable Z` property is enabled, particles will not move along the Z-axis. Whether that is going to be the particle system's local Z-axis or the world Z-axis is determined by the [Local Coords](https://docs.godotengine.org/en/stable/tutorials/3d/particles/properties.html#doc-3d-particles-properties-draw) property.
...
![Simple particle emission shapes](https://docs.godotengine.org/en/stable/_images/particle_shapes_simple.webp)

Particles emitting from a point (left), in a sphere (middle), and in a box (right) 

![Ring-shaped particle system](https://docs.godotengine.org/en/stable/_images/particle_ring.webp)
...
In addition to these relatively simple shapes, you can select the `Points` or `Directed Points` option to create highly complex emission shapes. See the [Complex emission shapes](https://docs.godotengine.org/en/stable/tutorials/3d/particles/complex_shapes.html#doc-3d-particles-complex-shapes) section for a detailed explanation of how to set these up.

## Angle

The `Angle` property controls a particle's starting rotation. In order to have an actual effect on the particle, you have to enable one of two properties: rotates the particle around the particle system's Y-axis. The `Billboard` property in the [Standard Material](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d), if it is set to `Particle Billboard`, rotates the particle around the axis that points from the particle to the camera.
...
![Different values for particle direction](https://docs.godotengine.org/en/stable/_images/particle_direction.webp)
...
![Different values for particle spread](https://docs.godotengine.org/en/stable/_images/particle_spread.webp)
...
![Different values for particle gravity](https://docs.godotengine.org/en/stable/_images/particle_gravity.webp)

Left: (X=0,Y=-9.8,Z=0), middle: (X=0,Y=9.8,Z=0), right: (X=4,Y=2,Z=0). 

## Angular velocity

`Angular Velocity` controls a particle's speed of rotation. You can reverse the direction by using negative numbers for `Velocity Min` or `Velocity Max`. Like the property, the rotation will only be visible if the flag is set or the `Particle Billboard` mode is selected in the [Standard Material](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d).

> [!note] Note
> The property has no effect on the angular velocity.

## Linear acceleration

A particle's velocity is a constant value: once it's set, it doesn't change and the particle will always move at the same speed. You can use the `Linear Accel` property to change the speed of movement over a particle's lifetime. Positive values will speed up the particle and make it move faster. Negative values will slow it down until it stops and starts moving in the other direction.

![Different values for particle linear acceleration](https://docs.godotengine.org/en/stable/_images/particle_accel_linear.webp)
...
![Different values for particle radial acceleration](https://docs.godotengine.org/en/stable/_images/particle_accel_radial.webp)

Negative (left) and positive (right) radial acceleration 

## Tangential acceleration

![Tangents on a circle](https://docs.godotengine.org/en/stable/_images/particle_tangent.webp)
...
![Tangential acceleration from above](https://docs.godotengine.org/en/stable/_images/particle_accel_tangent.webp)
...
`Scale` controls a particle's size. You can set different values for `Scale Min` and `Scale Max` to randomize each particle's size. Negative values are not allowed, so you won't be able to flip particles with this property. If you emit particles as billboards, the `Keep Size` property on the [Standard Material](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d) in your draw passes has to be enabled for any scaling to have an effect.

## Color

The `Color` property controls a particle's initial color. It will have an effect only after the `Use As Albedo` property in the `Vertex Color` group of the [Standard Material](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d) is enabled. This property is multiplied with color coming from the particle material's own `Color` or `Texture` property.

![Particle color ramp](https://docs.godotengine.org/en/stable/_images/particle_ramp.webp)
...
Hue describes a color's pigment: red, orange, yellow, green and so on. It does not tell you anything about how bright or how saturated the color is. The `Hue Variation` property controls the range of available hues.
...
![Different values for hue variation](https://docs.godotengine.org/en/stable/_images/particle_hue.webp)

Different values for hue variation, both times with blue as base color: 0.6 (left) and 0.1 (right) 

## Animation

The `Animation` property group controls the behavior of sprite sheet animations in the particle's [Standard Material](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d). The `Min`, `Max`, and `Curve` values work.
...
![A sprite sheet](https://docs.godotengine.org/en/stable/_images/particle_sprite.webp)

An 8x8 animated smoke sprite sheet 

1. Import a sprite sheet texture into the engine. If you don't have one at hand, you can download the [`high-res version of the example image`](https://docs.godotengine.org/en/stable/_downloads/0e9010cdc7bfae0d0bed55a967d66e0b/particle_sprite_smoke.webp).
...
![Animated particles](https://docs.godotengine.org/en/stable/_images/particle_animate.webp)
...
![Animated particles lifetimes](https://docs.godotengine.org/en/stable/_images/particle_animate_lifetime.webp)
...
Turbulence adds noise to particle movement, creating interesting and lively patterns. Check the box next to the `Enabled` property to activate it. A number of new properties show up that control the movement speed, noise pattern and overall influence on the particle system. You can find a detailed explanation of these in the section on [particle turbulence](https://docs.godotengine.org/en/stable/tutorials/3d/particles/turbulence.html#doc-3d-particles-turbulence).
...
If the `Use Scale` property is enabled, the [collision base size](https://docs.godotengine.org/en/stable/tutorials/3d/particles/properties.html#doc-3d-particles-properties-collision) is multiplied by the particle's. You can use this to make sure that the rendered size and the collision size match for particles with random scale or scale that varies over time.

You can learn more about particle collisions in the [Collisions](https://docs.godotengine.org/en/stable/tutorials/3d/particles/collision.html#doc-3d-particles-collision) section in this manual.

## Sub-emitter

![Sub-emitter modes](https://docs.godotengine.org/en/stable/_images/particle_sub_mode.webp)
...
See the [Sub-emitters](https://docs.godotengine.org/en/stable/tutorials/3d/particles/subemitters.html#doc-3d-particles-subemitters) section in this manual for a detailed explanation of how to add a sub-emitter to a particle system.
