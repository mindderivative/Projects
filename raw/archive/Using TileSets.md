...
created: 2026-06-19
description: "Introduction: A tilemap is a grid of tiles used to create a game's layout. There are several benefits to using TileMapLayer nodes to design your levels. First, they let you draw a layout by &q..."
words: 3569
tags:
  - 2d
  - tools
...
A tilemap is a grid of tiles used to create a game's layout. There are several benefits to using [TileMapLayer](https://docs.godotengine.org/en/stable/classes/class_tilemaplayer.html#class-tilemaplayer) nodes to design your levels. First, they let you draw a layout by "painting" tiles onto a grid, which is much faster than placing individual [Sprite2D](https://docs.godotengine.org/en/stable/classes/class_sprite2d.html#class-sprite2d) nodes one by one. Second, they allow for larger levels because they are optimized for drawing large numbers of tiles. Finally, they allow you to add greater functionality to your tiles with collision, occlusion, and navigation shapes.
...
This demonstration will use the following tiles taken from [Kenney's "Abstract Platformer" pack](https://kenney.nl/assets/abstract-platformer). We'll use this particular *tilesheet* from the set:

![Tilesheet example with 64×64 tiles](https://docs.godotengine.org/en/stable/_images/using_tilesets_kenney_abstract_platformer_tile_sheet.webp)
...
![Creating a new TileSet resource within the TileMapLayer node](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_new_tileset.webp)
...
![Setting the tile size to 64×64 to match the example tilesheet](https://docs.godotengine.org/en/stable/_images/using_tilesets_specify_size_then_edit.webp)
...
![Automatically creating tiles based on tilesheet image content](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_tiles_automatically.webp)
...
![Using the Eraser tool to remove unwanted tiles from the TileSet atlas](https://docs.godotengine.org/en/stable/_images/using_tilesets_eraser_tool.webp)
...
![Adjusting TileSet atlas properties in the dedicated inspector (part of the TileSet panel)](https://docs.godotengine.org/en/stable/_images/using_tilesets_properties.webp)
...
![Recreating tiles automatically after changing atlas properties](https://docs.godotengine.org/en/stable/_images/using_tilesets_recreate_tiles_automatically.webp)
...
> 
> It's recommended to only use scene tiles when necessary. To draw sprites in a tile without any kind of advanced manipulation,.
...
![Creating a scenes collection in the TileSet editor](https://docs.godotengine.org/en/stable/_images/using_tilesets_creating_scene_collection.webp)
...
![Creating a scene tile after selecting the scenes collection in the TileSet editor](https://docs.godotengine.org/en/stable/_images/using_tilesets_scene_collection_create_scene_tile.webp)
...
![Creating a scene slot, then loading a scene file into it in the TileSet editor](https://docs.godotengine.org/en/stable/_images/using_tilesets_adding_scene_tile.webp)
...
![Opening the atlas merging tool after creating multiple atlases](https://docs.godotengine.org/en/stable/_images/using_tilesets_open_atlas_merging_tool.webp)
...
![Using the atlas merging tool dialog](https://docs.godotengine.org/en/stable/_images/using_tilesets_atlas_merging_tool_dialog.webp)
...
If you use [2D navigation](https://docs.godotengine.org/en/stable/tutorials/navigation/navigation_introduction_2d.html#doc-navigation-overview-2d), you'll also need to define navigation polygons for tiles to generate a navigation mesh that agents can use for pathfinding.

Lastly, if you use [2D lights and shadows](https://docs.godotengine.org/en/stable/tutorials/2d/2d_lights_and_shadows.html#doc-2d-lights-and-shadows) or GPUParticles2D, you may also want your TileSet to be able to cast shadows and collide with particles. This requires defining occluder polygons for "solid" tiles on the TileSet.
...
![Creating a physics layer in the TileSet resource inspector (within the TileMapLayer node)](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_physics_layer.webp)
...
![Creating a navigation layer in the TileSet resource inspector (within the TileMapLayer node)](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_navigation_layer.webp)
...
![Creating an occlusion layer in the TileSet resource inspector (within the TileMapLayer node)](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_occlusion_layer.webp)
...
![Opening the collision editor while in Select mode](https://docs.godotengine.org/en/stable/_images/using_tilesets_selecting_collision_editor.webp)
...
![Using default rectangle collision shape by pressing :kbd:`F`](https://docs.godotengine.org/en/stable/_images/using_tilesets_using_default_rectangle_collision.webp)
...
![Creating a triangle collision shape by right-clicking one of the corners to remove it](https://docs.godotengine.org/en/stable/_images/using_tilesets_creating_triangle_collision.webp)
...
![Drawing a custom collision for a complex tile shape](https://docs.godotengine.org/en/stable/_images/using_tilesets_drawing_custom_collision.webp)

Drawing a custom collision for a complex tile shape 

> [!tip] Tip
> If you have a large tileset, specifying the collision for each tile individually could take a lot of time. This is especially true as TileMaps tend to have many tiles with common collision patterns (such as solid blocks or 45-degree slopes). To apply a similar collision shape to several tiles quickly, use functionality to.
...
![Creating a terrain set in the TileSet resource inspector (within the TileMapLayer node)](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_terrain_set.webp)
...
![Creating a terrain within the terrain set](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_terrain.webp)
...
![Configuring terrain on a single tile in the TileSet editor's Select mode](https://docs.godotengine.org/en/stable/_images/using_tilesets_configure_terrain_on_tile.webp)
...
![Configuring terrain peering bits on a single tile in the TileSet editor's Select mode](https://docs.godotengine.org/en/stable/_images/using_tilesets_configure_terrain_peering_bits.webp)
...
![Example full tilesheet for a sidescrolling game](https://docs.godotengine.org/en/stable/_images/using_tilesets_terrain_example_tilesheet.webp)

Example full tilesheet for a sidescrolling game 

![Example full tilesheet for a sidescrolling game with terrain peering bits visible](https://docs.godotengine.org/en/stable/_images/using_tilesets_terrain_example_tilesheet_configuration.webp)
...
![Selecting multiple tiles using the Select mode, then applying properties](https://docs.godotengine.org/en/stable/_images/using_tilesets_select_and_set_tile_properties.webp)
...
![Painting tile properties using the TileSet editor](https://docs.godotengine.org/en/stable/_images/using_tilesets_paint_tile_properties.webp)
...
![Painting a collision polygon, then left-clicking tiles to apply it](https://docs.godotengine.org/en/stable/_images/using_tilesets_paint_tile_properties_collision.webp)
...
![Creating an alternative tile by right-clicking a base tile in the TileSet editor](https://docs.godotengine.org/en/stable/_images/using_tilesets_create_alternative_tile.webp)
...
![Configuring an alternative tile after clicking it in the TileSet editor](https://docs.godotengine.org/en/stable/_images/using_tilesets_configure_alternative_tile.webp)
