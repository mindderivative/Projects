...
source: "https://docs.godotengine.org/en/stable/tutorials/3d/lights_and_shadows.html#introduction"
created: 2026-06-20
description: "Introduction: Light sources emit light that mixes with the materials and produces a visible result. Light can come from several types of sources in a scene: From the material itself, in the form of..."
words: 4415
---
## Introduction
...
- Ambient light in the [Environment](https://docs.godotengine.org/en/stable/classes/class_environment.html#class-environment) or [Reflection probes](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/reflection_probes.html#doc-reflection-probes).
- Global illumination ([LightmapGI](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_lightmap_gi.html#doc-using-lightmap-gi), [VoxelGI](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_voxel_gi.html#doc-using-voxel-gi) or [SDFGI](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_sdfgi.html#doc-using-sdfgi)).

The emission color is a material property. You can read more about it in the [Standard Material 3D and ORM Material 3D](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#doc-standard-material-3d) tutorial.

> [!note] See also
> You can compare various types of lights in action using the [3D Lights and Shadows demo project](https://github.com/godotengine/godot-demo-projects/tree/master/3d/lights_and_shadows).

## Light nodes

There are three types of light nodes: [DirectionalLight3D](https://docs.godotengine.org/en/stable/classes/class_directionallight3d.html#class-directionallight3d), [OmniLight3D](https://docs.godotengine.org/en/stable/classes/class_omnilight3d.html#class-omnilight3d) and [SpotLight3D](https://docs.godotengine.org/en/stable/classes/class_spotlight3d.html#class-spotlight3d). Let's take a look at the common parameters for lights:

![../../_images/light_params.png](https://docs.godotengine.org/en/stable/_images/light_params.png)

Each property has a specific function:

- **Color:** Base color for emitted light.
- **Energy:** Energy multiplier. This is useful for saturating lights or working with [High dynamic range lighting](https://docs.godotengine.org/en/stable/tutorials/3d/high_dynamic_range.html#doc-high-dynamic-range).
- **Indirect Energy:** Secondary multiplier used with indirect light (light bounces). This works with [Using Lightmap global illumination](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_lightmap_gi.html#doc-using-lightmap-gi), VoxelGI or SDFGI.
...
- **Bake Mode:** Sets the bake mode for the light. See [Using Lightmap global illumination](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_lightmap_gi.html#doc-using-lightmap-gi).
...
> [!note] See also
> See [Physical light and camera units](https://docs.godotengine.org/en/stable/tutorials/3d/physical_light_and_camera_units.html#doc-physical-light-and-camera-units) if you wish to use real world units to configure your lights' intensity and color temperature.

## Light number limits

When using the Forward+ renderer, Godot uses a *clustering* approach for real-time lighting. As many lights as desired can be added (as long as performance allows). However, there's still a default limit of 512 *clustered elements* that can be present in the current camera view. A clustered element is an omni light, a spot light, a [decal](https://docs.godotengine.org/en/stable/tutorials/3d/using_decals.html#doc-using-decals) or a [reflection probe](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/reflection_probes.html#doc-reflection-probes). This limit can be increased by adjusting [Max Clustered Elements](https://docs.godotengine.org/en/stable/classes/class_projectsettings.html#class-projectsettings-property-rendering-limits-cluster-builder-max-clustered-elements) in **Project Settings > Rendering > Limits > Cluster Builder**.
...
When using the Compatibility renderer, up to 8 OmniLights + 8 SpotLights can be rendered per mesh resource. This limit can be increased in the advanced Project Settings by adjusting [Max Renderable Elements](https://docs.godotengine.org/en/stable/classes/class_projectsettings.html#class-projectsettings-property-rendering-limits-opengl-max-renderable-elements) and/or [Max Lights per Object](https://docs.godotengine.org/en/stable/classes/class_projectsettings.html#class-projectsettings-property-rendering-limits-opengl-max-lights-per-object) in **Rendering > Limits > OpenGL**, at the cost of performance and longer shader compilation times. The limit can also be decreased to reduce shader compilation times and improve performance slightly.
...
If you need to render more lights than possible in a given renderer, consider using [baked lightmaps](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_lightmap_gi.html#doc-using-lightmap-gi) with lights' bake mode set to **Static**. This allows lights to be fully baked, which also makes them much faster to render. You can also use emissive materials with any [global illumination](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/introduction_to_global_illumination.html#doc-introduction-to-global-illumination) technique as a replacement for light nodes that emit light over a large area.
...
- **Blur:** Multiplies the shadow blur radius for this light. This works with both traditional shadow mapping and contact-hardening shadows (lights with **Angular Distance** or **Size** greater than `0.0`). Higher values result in softer shadows, which will also appear to be more temporally stable for moving objects. The downside of increasing shadow blur is that it will make the grainy pattern used for filtering more noticeable. See also.
...
![../../_images/lights_and_shadows_blur.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_blur.webp)
...
![../../_images/lights_and_shadows_acne.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_acne.webp)
...
![../../_images/lights_and_shadows_peter_panning.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_peter_panning.webp)
...
Any sort of bias issues can be fixed by, at the cost of decreased performance.
...
![../../_images/light_directional.png](https://docs.godotengine.org/en/stable/_images/light_directional.png)
...
For reference, the angular distance of the Sun viewed from the Earth is approximately `0.5`. This kind of shadow is expensive, so check the recommendations in if setting this value above `0.0` on lights with shadows enabled.
...
![../../_images/lights_and_shadows_pssm_explained.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_pssm_explained.webp)

With this, shadows become more detailed:

![../../_images/lights_and_shadows_directional_mode.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_directional_mode.webp)

To control PSSM, a number of parameters are exposed:

![../../_images/lights_and_shadows_directional_shadow_params.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_directional_shadow_params.webp)
...
![../../_images/blend_splits.png](https://docs.godotengine.org/en/stable/_images/blend_splits.png)
...
![../../_images/light_omni.png](https://docs.godotengine.org/en/stable/_images/light_omni.png)
...
![../../_images/lights_and_shadows_dual_parabolid_vs_cubemap.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_dual_parabolid_vs_cubemap.webp)
...
Omni light projector textures require a special 360° panorama mapping, similar to [PanoramaSkyMaterial](https://docs.godotengine.org/en/stable/classes/class_panoramaskymaterial.html#class-panoramaskymaterial) textures.

With the projector texture below, the following result is obtained:

![../../_images/lights_and_shadows_omni_projector_example.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_omni_projector_example.webp) ![../../_images/lights_and_shadows_omni_projector.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_omni_projector.webp)

> [!tip] Tip
> If you've acquired omni projectors in the form of cubemap images, you can use [this web-based conversion tool](https://danilw.github.io/GLSL-howto/cubemap_to_panorama_js/cubemap_to_panorama.html) to convert them to a single panorama image.
...
Unlike omni light projectors, a spot light projector texture doesn't need to follow a special format to look correct. It will be mapped in a way similar to a [decal](https://docs.godotengine.org/en/stable/tutorials/3d/using_decals.html#doc-using-decals).

With the projector texture below, the following result is obtained:

![../../_images/lights_and_shadows_spot_projector_example.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_spot_projector_example.webp) ![../../_images/lights_and_shadows_spot_projector.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_spot_projector.webp)
...
Godot provides the [AreaLight3D](https://docs.godotengine.org/en/stable/classes/class_arealight3d.html#class-arealight3d) node for this purpose, which emits light from a rectangular area. The node only emits light and has no other visual representation in the scene. The screenshots below use a [Sprite3D](https://docs.godotengine.org/en/stable/classes/class_sprite3d.html#class-sprite3d) node as a child of the area light for visualization purposes.
...
Area lights can also cast shadows, with variable penumbra simulated using by default. The size of this penumbra can be controlled with the Light3D **Size** property. This effect can be quite demanding, so it can be turned off by setting **Size** to `0.0`.
...
![../../_images/lights_and_shadows_area_example.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_area_example.webp)
...
![../../_images/lights_and_shadows_area_texture.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_area_texture.webp)
...
![../../_images/lights_and_shadows_shadow_quadrants.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_shadow_quadrants.webp)

Each quadrant can be subdivided to allocate any number of shadow maps; the following is the default subdivision:

![../../_images/lights_and_shadows_shadow_quadrants2.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_shadow_quadrants2.webp)
...
Directional shadow quality settings can be changed at runtime by calling the appropriate [RenderingServer](https://docs.godotengine.org/en/stable/classes/class_renderingserver.html#class-renderingserver) methods.

Positional (omni/spot) shadow quality settings can be changed at runtime on the root [Viewport](https://docs.godotengine.org/en/stable/classes/class_viewport.html#class-viewport).
...
If positional shadows become too blurry after decreasing the shadow map size, you can counteract this by adjusting the quadrants to contain fewer shadows. This will allow each shadow to be rendered at a higher resolution.
...
However, in projects with less detailed textures, the shadow dithering pattern may be more visible. To hide this pattern, you can either enable [Temporal antialiasing (TAA)](https://docs.godotengine.org/en/stable/tutorials/3d/3d_antialiasing.html#doc-3d-antialiasing-taa), [AMD FidelityFX Super Resolution 2.2 (FSR2)](https://docs.godotengine.org/en/stable/tutorials/3d/3d_antialiasing.html#doc-3d-antialiasing-fsr2), [Fast approximate antialiasing (FXAA)](https://docs.godotengine.org/en/stable/tutorials/3d/3d_antialiasing.html#doc-3d-antialiasing-fxaa), or increase the shadow filter quality to **Soft Medium** or higher.
...
![../../_images/lights_and_shadows_filter_quality.webp](https://docs.godotengine.org/en/stable/_images/lights_and_shadows_filter_quality.webp)
