...
source: "https://docs.godotengine.org/en/stable/tutorials/3d/environment_and_post_processing.html#environment"
created: 2026-06-20
description: "Godot 4 provides a redesigned Environment resource, as well as a new post-processing system with many available effects right out of the box. Environment: The Environment resource stores all the in..."
words: 6600
---
...
![../../_images/environment_preview_sun_sky_toggle.webp](https://docs.godotengine.org/en/stable/_images/environment_preview_sun_sky_toggle.webp)
...
![../../_images/environment_preview_sun_sky_dialog.webp](https://docs.godotengine.org/en/stable/_images/environment_preview_sun_sky_dialog.webp)
...
The [CameraAttributes](https://docs.godotengine.org/en/stable/classes/class_cameraattributes.html#class-cameraattributes) resource stores exposure and depth of field information. It also allows enabling automatic exposure adjustments depending on scene brightness.
...
> [!note] Note
> Using a [CameraAttributesPhysical](https://docs.godotengine.org/en/stable/classes/class_cameraattributesphysical.html#class-cameraattributesphysical) on a Camera3D node will lock out FOV and aspect adjustments in that Camera3D, as field of view is adjusted in the CameraAttributesPhysical resource instead. If used in a WorldEnvironment, the CameraAttributesPhysical will not override any Camera3D in the scene.
...
![../../_images/environment_background1.webp](https://docs.godotengine.org/en/stable/_images/environment_background1.webp)
...
- **Canvas** displays the 2D scene as a background to the 3D scene. This can be used to make environment effects visible on 2D rendering, such as.
...
- **Camera Feed** displays a [CameraFeed](https://docs.godotengine.org/en/stable/classes/class_camerafeed.html#class-camerafeed) from a physical camera as the background, useful for AR games on mobile devices.
...
Panorama sky images are sometimes called HDRIs (High Dynamic Range Images). You can find freely licensed HDRIs on [Poly Haven](https://polyhaven.com/hdris).
...
If you need a custom sky material (e.g. for procedural clouds), you can create a custom [sky shader](https://docs.godotengine.org/en/stable/tutorials/shaders/shader_reference/sky_shader.html#doc-sky-shader).
...
![../../_images/environment_ambient2.webp](https://docs.godotengine.org/en/stable/_images/environment_ambient2.webp)

Finally, there is an **Energy** setting which is a multiplier. It's useful when working with HDR.

In general, you should only rely on ambient light alone for simple scenes or large exteriors. You may also do so to boost performance. Ambient light is fast to render, but it doesn't provide the best lighting quality. It's better to generate ambient light from [ReflectionProbe](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/reflection_probes.html#doc-reflection-probes), [VoxelGI](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_voxel_gi.html#doc-using-voxel-gi) or [SDFGI](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_sdfgi.html#doc-using-sdfgi), as these will simulate how indirect light propagates more accurately. Below is a comparison, in terms of quality, between using a flat ambient color and a VoxelGI:

![../../_images/environment_ambient_comparison.webp](https://docs.godotengine.org/en/stable/_images/environment_ambient_comparison.webp)
...
- **Mode:** The tonemapping mode to use.
	- **Linear:** Does not modify color data, resulting in a linear tonemapping curve which unnaturally clips bright values, causing bright lighting to look blown out. The simplest and fastest tonemapper.
		...
		- **AgX:** Uses a film-like tonemapping curve and desaturates bright values for a more realistic appearance. Better than other tonemappers at maintaining the hue of colors as they become brighter. The slowest tonemapping option.
...
While Godot supports several sources of reflection data such as [Reflection probes](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/reflection_probes.html#doc-reflection-probes), they may not provide enough detail for all situations. Scenarios where screen-space reflections make the most sense are when objects are in contact with each other (object over floor, over a table, floating on water, etc).

![../../_images/environment_ssr.webp](https://docs.godotengine.org/en/stable/_images/environment_ssr.webp)
...
![../../_images/environment_ssao.webp](https://docs.godotengine.org/en/stable/_images/environment_ssao.webp)
...
![../../_images/environment_ssao2.webp](https://docs.godotengine.org/en/stable/_images/environment_ssao2.webp)

Tweaking SSAO is possible with several parameters:

![../../_images/environment_ssao_parameters.webp](https://docs.godotengine.org/en/stable/_images/environment_ssao_parameters.webp)
...
Additionally, you can adjust the quality of SSAO in the project settings' **Rendering > Environment > SSAO** section:

- **Quality:** Sets the quality of the screen-space ambient occlusion effect. Higher values take more samples and so will result in better quality at the cost of performance. Setting this to Ultra will use the **Adaptive Target** setting (see below).
- **Half Size:** If `true`, screen-space ambient occlusion will be rendered at half size and then upscaled before being added to the scene. This is significantly faster but may miss small details. If `false`, screen-space ambient occlusion will be rendered at full size.
- **Adaptive Target:** Quality target to use when **Quality** is set to **Ultra**. A value of `0.0` provides a quality and speed similar to Medium while a value of `1.0` provides much higher quality than any of the other settings at the cost of performance.
- **Blur Passes:** Number of blur passes to use when computing screen-space ambient occlusion. A higher number will result in a smoother look, but will be slower to compute and will have less high-frequency detail.
- **Fadeout From:** Distance at which the screen-space ambient occlusion effect starts to fade out. Use this hide ambient occlusion from far away.
- **Fadeout To:** Distance at which the screen-space ambient occlusion is fully faded out. Use this hide ambient occlusion from far away.

> [!note] Note
> Since Godot 4.6, a simplified version of SSAO is available in the Compatibility renderer. This implementation has a different look, but should perform significantly better on low-end devices compared to SSAO in Forward+.
> 
> When using the Compatibility renderer, only the **Radius** and **Intensity** parameters can be adjusted.
...
Additionally, you can adjust the quality of SSIL in the project settings' **Rendering > Environment > SSIL** section:

- **Quality:** Sets the quality of the screen-space ambient occlusion effect. Higher values take more samples and so will result in better quality, at the cost of performance. Setting to Ultra will use the **Adaptive Target** setting (see below).
- **Half Size:** If `true`, screen-space ambient occlusion will be rendered at half size and then upscaled before being added to the scene. This is significantly faster but may miss small details. If `false`, screen-space ambient occlusion will be rendered at full size.
- **Adaptive Target:** Quality target to use when **Quality** is set to **Ultra**. A value of `0.0` provides a quality and speed similar to Medium while a value of `1.0` provides much higher quality than any of the other settings at the cost of performance. When using the adaptive target, the performance cost scales with the complexity of the scene.
- **Blur Passes:** Number of blur passes to use when computing screen-space indirect lighting. A higher number will result in a smoother look, but will be slower to compute and will have less high-frequency detail.
- **Fadeout From:** Distance at which the screen-space indirect lighting effect starts to fade out. Use this to hide screen-space indirect lighting from far away.
- **Fadeout To:** Distance at which the screen-space indirect lighting is fully faded out. Use this to hide screen-space indirect lighting from far away.
![../../_images/environment_ssil.webp](https://docs.godotengine.org/en/stable/_images/environment_ssil.webp)
...
> [!note] See also
> See [Signed distance field global illumination (SDFGI)](https://docs.godotengine.org/en/stable/tutorials/3d/global_illumination/using_sdfgi.html#doc-using-sdfgi) for instructions on setting up this global illumination technique.

![../../_images/environment_sdfgi.webp](https://docs.godotengine.org/en/stable/_images/environment_sdfgi.webp)
...
![../../_images/environment_glow1.webp](https://docs.godotengine.org/en/stable/_images/environment_glow1.webp)
...
![../../_images/environment_glow_threshold.webp](https://docs.godotengine.org/en/stable/_images/environment_glow_threshold.webp)
...
![../../_images/environment_glow_bloom.webp](https://docs.godotengine.org/en/stable/_images/environment_glow_bloom.webp)
...
- **Replace** can be used to or debug the effect. It only shows the glow effect without the image below.
...
![../../_images/environment_glow_layers.webp](https://docs.godotengine.org/en/stable/_images/environment_glow_layers.webp)

The real strength of this system, though, is to combine levels to create more interesting glow patterns:

![../../_images/environment_glow_layers2.webp](https://docs.godotengine.org/en/stable/_images/environment_glow_layers2.webp)
...
![../../_images/environment_glow_map.webp](https://docs.godotengine.org/en/stable/_images/environment_glow_map.webp)
...
![../../_images/environment_and_post_processing_glow_scale_mode.webp](https://docs.godotengine.org/en/stable/_images/environment_and_post_processing_glow_scale_mode.webp)
...
- Since Godot 4.2, you can enable HDR for 2D rendering when using the Forward+ and Mobile rendering methods. This has a performance cost, but it allows for a greater dynamic range. This also allows you to control which objects glow using their individual **Modulate** or **Self Modulate** properties (use the Intensity slider in the color picker). Enabling HDR can also reduce banding in the 2D rendering output.
	- To enable HDR in 2D, open the Project Settings, enable [Rendering > Viewport > HDR 2D](https://docs.godotengine.org/en/stable/classes/class_projectsettings.html#class-projectsettings-property-rendering-viewport-hdr-2d) then restart the editor.
- If you want to maximize performance, you can leave HDR disabled for 2D rendering. However, you will have less control on which objects glow.
	- Enable glow, set the environment background mode to **Canvas** then decrease **Glow HDR Threshold** so that pixels that are not overbright will still glow. To prevent UI elements from glowing, make them children of a [CanvasLayer](https://docs.godotengine.org/en/stable/classes/class_canvaslayer.html#class-canvaslayer) node. You can control which layers are affected by glow using the **Background > Canvas Max Layer** property of the Environment resource.
![Example of using glow in a 2D scene](https://docs.godotengine.org/en/stable/_images/environment_and_post_processing_glow_in_2d.webp)

Example of using glow in a 2D scene. HDR 2D is enabled, while coins and the bullet have their Modulate property increased to overbright values using the Intensity slider in the color picker. 

> [!warning] Warning
> The 2D renderer renders in linear color space if the [Rendering > Viewport > HDR 2D](https://docs.godotengine.org/en/stable/classes/class_projectsettings.html#class-projectsettings-property-rendering-viewport-hdr-2d) project setting is enabled, so the `source_color` hint must also be used for uniform samplers that are used as color input in `canvas_item` shaders. If this is not done, the texture will appear washed out.
> 
...
![Example of using glow to blur the 2D rendering in the menu's background](https://docs.godotengine.org/en/stable/_images/environment_and_post_processing_glow_blur.webp)
...
> [!note] Note
> This section refers to non-volumetric fog only. It is possible to use both non-volumetric fog and [Volumetric fog and fog volumes](https://docs.godotengine.org/en/stable/tutorials/3d/volumetric_fog.html#doc-volumetric-fog) at the same time.
...
![../../_images/environment_fog_depth_height.webp](https://docs.godotengine.org/en/stable/_images/environment_fog_depth_height.webp)
...
The first is **Sun Scatter**, which makes use of the DirectionalLight3D's color and energy in the current scene. When looking toward the directional light (usually a sun), the fog will be tinted according to the light's color to simulate the sunlight passing through the fog.
...
> [!note] Note
> Fog can cause banding to appear on the viewport, especially at higher density levels. See [Color banding](https://docs.godotengine.org/en/stable/tutorials/3d/3d_rendering_limitations.html#doc-3d-rendering-limitations-color-banding) for guidance on reducing banding.
...
> [!note] See also
> See [Volumetric fog and fog volumes](https://docs.godotengine.org/en/stable/tutorials/3d/volumetric_fog.html#doc-volumetric-fog) for documentation on setting up volumetric fog.

### Adjustments

At the end of processing, Godot offers the possibility to do some standard image adjustments.

![../../_images/environment_adjustments.webp](https://docs.godotengine.org/en/stable/_images/environment_adjustments.webp)
...
![../../_images/environment_adjustments_bcs.webp](https://docs.godotengine.org/en/stable/_images/environment_adjustments_bcs.webp)
...
![../../_images/environment_adjustments_custom_gradient.webp](https://docs.godotengine.org/en/stable/_images/environment_adjustments_custom_gradient.webp)
...
![../../_images/environment_adjustments_3d_lut_import.webp](https://docs.godotengine.org/en/stable/_images/environment_adjustments_3d_lut_import.webp)
...
![../../_images/environment_adjustments_3d_lut_comparison.webp](https://docs.godotengine.org/en/stable/_images/environment_adjustments_3d_lut_comparison.webp)
...
![../../_images/environment_dof_far.webp](https://docs.godotengine.org/en/stable/_images/environment_dof_far.webp)
...
![../../_images/environment_dof_near.webp](https://docs.godotengine.org/en/stable/_images/environment_dof_near.webp)

The **Amount** parameter controls the amount of blur. For larger blurs, tweaking the depth of field quality in the advanced project settings may be needed to avoid artifacts.

It is common to use both blurs together to focus the viewer's attention on a given object, or create a so-called ["tilt shift" effect](https://en.wikipedia.org/wiki/Miniature_faking).

![../../_images/environment_mixed_blur.webp](https://docs.godotengine.org/en/stable/_images/environment_mixed_blur.webp)
...
![../../_images/environment_hdr_autoexp.webp](https://docs.godotengine.org/en/stable/_images/environment_hdr_autoexp.webp)
...
By combining Auto Exposure with post-processing, pixels that go over the tonemap **White** will bleed to the glow buffer, creating the typical bloom effect in photography.

![../../_images/environment_hdr_bloom.webp](https://docs.godotengine.org/en/stable/_images/environment_hdr_bloom.webp)

The user-controllable values in the Auto Exposure section come with sensible defaults, but you can still tweak them:

![../../_images/environment_hdr.webp](https://docs.godotengine.org/en/stable/_images/environment_hdr.webp)
...
> [!note] See also
> See [Physical light and camera units](https://docs.godotengine.org/en/stable/tutorials/3d/physical_light_and_camera_units.html#doc-physical-light-and-camera-units) if you wish to use real world units to configure your camera's exposure, field of view and depth of field.
