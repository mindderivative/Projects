...
@export var spawning_component : SpawningComponent2D
...
func start() -> void:
	...
	call_deferred("append_max_objects")
...
func add_object() -> void:
	...
	var obj : Node2D = data.scene.instantiate()
	
	obj.set_process(false)
	obj.set_physics_process(false)
	obj.hide()
	object_pool.append(obj)
	#obj.global_position = Entity.global_position
	if spawning_component != null:
		spawning_component.add_child(obj, true)
	else:
		add_child(obj, true)
#endregion
