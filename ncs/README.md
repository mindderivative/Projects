...
## 📝 Documentation Workflow (Obsidian + VS Code)

Project documentation is maintained as markdown notes that are compatible with both Obsidian and VS Code.

* Treat repository `.md` files as vault notes.
* Keep note links stable by using wiki links (`[[Note Name]]`) or relative markdown links.
* When changing architecture, public behavior, APIs, or developer workflow, update documentation in the same change.
* Include a brief migration or integration note when behavior changes could affect existing scenes/components.

Recommended extensions in this workflow include Obsidian MD for VSCode, Obsidian Code Sync, Foam, Markdown All in One, VSCode Editor Plugin, and Open Vault in VSCode.
